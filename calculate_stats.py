...
with open("fetch_usatoday.csv", "r", encoding="UTF-8") as f:
...
with open("visit_usatoday.csv", "r", encoding="UTF-8") as f:
    data = pd.read_csv(f, header=0)
...
with open("urls_usatoday.csv", "r", encoding="UTF-8") as f:
    data = pd.read_csv(f, header=0)
    unique_extracted = data.shape[0]
    unique_within = data[data["Status"] == "OK"].shape[0]
    unique_outside = data[data["Status"] == "N_OK"].shape[0]

with open("CrawlReport_usatoday.txt", "w") as f:
    f.write(f"Name: Dipali Telavane\n")
    f.write(f"USC ID: 1487027774\n")
